...
```python {.marimo hide_code="true"}
mo.md(f"""
Here, we plot the value function of all the states in the Gridworld.

The {slider} allows to view the evolution of the value function over iterations.
...
Let's run the policy iteration algorithm.

```python {.marimo}
policy_iteration = PolicyIteration(env, gamma=0.9, epsilon=1e-8)
v_policy, pi_policy = policy_iteration.run()
```
...
The {slider_pi} allows to view the evolution of the value function $v_\pi$ and the associated policy $\pi$ over the course of the convergence of the algorithm.
